...
if __name__ == '__main__':
    print (
    '{} (TM) Configurable Multi Module Python Obfuscator Version {}'.format(programName.capitalize(), programVersion))
    ...
    # Changes unichr to unichar, aviod coding error when run in windows.
    def unichar(i):
        try:
            return unichr(i)
        except ValueError:
    ...
    def scramble(stringLiteral):
        ...
        if isPython2:
            recodedStringLiteral = unicode().join(
                [unichar(charBase + ord(char) + (charIndex + stringNr) % charModulus) for charIndex, char in
                 enumerate(stringLiteral)])
            stringKey = unichar(stringNr)
        else:
            recodedStringLiteral = str().join(
                [chr(charBase + ord(char) + (charIndex + stringNr) % charModulus) for charIndex, char in
                 enumerate(stringLiteral)])
...
*******************************************************************************
    ...
    excludePathPartList = exclude_paths.split()
    ...
    def getDecodedStringPlaceholderAndRegister(matchObject):
        ...
        if obfuscateStrings:
            if keepStringRegEx.search(string):  # Rare, so no need for speed
                ...
                return 'unScramble{0} ({1})'.format(plainMarker,
                                                    stringPlaceholder)  # Store unScramble (<scrambledString>)
        else:
    ...
    def moveFromFuture(matchObject):
        ...
        if fromFuture:
            ...
            contentList[nrOfSpecialLines:nrOfSpecialLines] = [
                fromFuture]  # Move 'from __future__' line after other special lines
            nrOfSpecialLines += 1
        return ''
    ...
    skipWordSet = set(
        keyword.kwlist + ['__init__'] + extraPlainWordList)  # __init__ should be in, since __init__.py is special

    plainFilePathList = ['{0}/{1}'.format(sourceRootDirectory, plainFileRelPath) for plainFileRelPath in
                         plainFileRelPathList]
    ...
    class ExternalModules:
        def __init__(self):
            for externalModuleName in externalModuleNameList:
                attributeName = externalModuleName.replace('.',
                                                           plainMarker)  # Replace . in module name by placeholder to get attribute name
                try:
                    exec (
                        '''
import {0} as currentModule
    ...
    def addExternalNames(anObject):
        ...
        attributeSkipWordList = (plainMarker.join(attributeNameList)).split(
            plainMarker)  # Split module name chunks that were joined by placeholder

        updateSet = set([entry for entry in (parameterNameList + attributeSkipWordList) if
                         not (entry.startswith('__') and entry.endswith('__'))])
    ...
    for sourceFilePath in sourceFilePathList:
        if any([excludename in sourceFilePath for excludename in excludePathPartList]):
            continue
        ...
        # Fixed the occasional absence of path not obfuscated.
        sourceDirectoryPreName = (sourceDirectory.rsplit('/', 1))[-1]
        ...
        if sourceFileNameExtension in sourceFileNameExtensionList and not sourceFilePath in plainFilePathList:
            ...
            if len(contentList) > 0:
                if shebangCommentRegEx.search(contentList[0]):  # If the original code starts with a shebang line
                    ...
                    if len(contentList) > 1 and codingCommentRegEx.search(
                            contentList[1]):  # If after the shebang a coding comment follows
                        nrOfSpecialLines += 1  # Account for that
                        insertCodingComment = False  # Don't insert, it's already there
                elif codingCommentRegEx.search(
                        contentList[0]):  # Else if the original code starts with a coding comment
                    nrOfSpecialLines += 1  # Account for that
                    insertCodingComment = False  # Don't insert, it's already there

            if obfuscateStrings and insertCodingComment:  # Obfuscated strings are always converted to unicode
                contentList[nrOfSpecialLines:nrOfSpecialLines] = [
                    '# coding: UTF-8']  # Insert the coding line if it wasn't there
            ...
            # if sourceDirectory.rsplit("/", 1)[0] != sourceRootDirectory:
            #     print(sourceDirectory.rsplit("/", 1)[-1])
            #     sourceWordSet = set(re.findall(identifierRegEx, normalContent) + [sourceFilePreName] + [sourceDirectory.rsplit("/", 1)[-1]])
            # else:
            sourceWordSet = set(re.findall(identifierRegEx, normalContent) + [sourceFilePreName] + [sourceDirectoryPreName])
            ...
            strippedSourceWordSet = sourceWordSet.difference(obfuscatedWordList).difference(
                skipWordSet)  # Leave out what is already or shouldn't be obfuscated
            strippedSourceWordList = list(strippedSourceWordSet)
            strippedSourceRegExList = [re.compile(r'\b{0}\b'.format(sourceWord)) for sourceWord in
                                       strippedSourceWordList]  # Regex used to replace obfuscated words
            ...
            # print(obfuscatedWordList.index("SaaSTools"))
            ...
            # if "SaaSTools" in targetChunks:
            #     print("sssss" * 10, len(targetChunks), targetRelSubDirectory)
            # else:
            #     print("mmmmm" * 10, len(targetChunks), targetRelSubDirectory)
            for index in range(len(targetChunks)):
                try:
                    targetChunks[index] = getObfuscatedName(obfuscatedWordList.index(targetChunks[index]),
                                                            targetChunks[index])
                except:  # Not in list
                    pass
            ...
            print('{0} -----> {1}/{2}.{3}' .format(sourceFilePath, targetSubDirectory, targetFilePreName, sourceFileNameExtension))
            targetFile = createFilePath(
                '{0}/{1}.{2}'.format(targetSubDirectory, targetFilePreName, sourceFileNameExtension), open=True)
